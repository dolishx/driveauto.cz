...
export default async function HomePage() {
  ...
  return (
    <div className="bg-white">
      <section className="relative overflow-hidden border-b border-brand-line bg-[radial-gradient(circle_at_80%_20%,rgba(7,95,232,0.10),transparent_34%),linear-gradient(180deg,#ffffff_0%,#f7faff_100%)]">
        <div className="mx-auto grid max-w-7xl items-center gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[0.44fr_0.56fr] lg:px-8 lg:py-12">
          <div className="relative z-10 max-w-[620px]">
            ...
            <h1 className="mt-4 text-5xl font-bold leading-[0.98] tracking-[-0.055em] text-brand-navy lg:text-[64px]">
          ...
          <div className="relative h-80 overflow-hidden rounded-3xl bg-brand-soft shadow-[0_24px_60px_rgba(8,23,52,0.14)] lg:h-[430px]">
            <Image
            ...
            <div className="absolute bottom-5 left-5 right-5 rounded-2xl border border-white/25 bg-white/90 p-4 shadow-[0_18px_40px_rgba(8,23,52,0.16)] backdrop-blur">
      ...
      <section id="sluzby" className="border-y border-brand-line bg-brand-soft/60">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-16">
          <div>
            ...
            <h2 className="mt-4 max-w-xl text-4xl font-bold tracking-[-0.055em] text-brand-navy md:text-5xl">
            ...
            {customerSituations.map((item) => {
              ...
              return (
                <Link
                  key={item.title}
                  href={item.href}
                  className="group rounded-2xl border border-brand-line bg-white p-6 shadow-[0_14px_34px_rgba(8,23,52,0.06)] transition hover:-translate-y-1 hover:border-brand-blue/30 hover:shadow-[0_18px_45px_rgba(8,23,52,0.10)]"
                >
                  ...
                  <p className="mt-5 text-xs font-bold uppercase tracking-wide text-brand-blue">
                    {item.meta}
                  </p>
                  <h3 className="mt-3 text-xl font-bold tracking-[-0.03em] text-brand-navy">
                    {item.title}
                  </h3>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-12 sm:px-6 lg:px-8">
        ...
        <div className="relative grid gap-4 lg:grid-cols-5">
          ...
          {processSteps.map((step, index) => (
            <article key={step.title} className="relative rounded-2xl border border-brand-line bg-white p-5 shadow-[0_12px_30px_rgba(8,23,52,0.05)]">
              <span className="flex h-12 w-12 items-center justify-center rounded-full bg-brand-blue text-sm font-bold text-white shadow-[0_12px_24px_rgba(7,95,232,0.22)]">
                {index + 1}
              </span>
              <h3 className="mt-5 text-lg font-bold tracking-[-0.03em] text-brand-navy">
                {step.title}
              </h3>
              <p className="mt-3 text-sm leading-6 text-brand-muted">
                {step.description}
